...
void* windowNew(unsigned int id, int width, int height, bool fraudulentWebsiteWarningEnabled, bool frameless, bool squareCorners, double cornerRadius, bool enableDragAndDrop, struct WebviewPreferences preferences, const char* applicationNameForUserAgent) {
	...
	// wps-enhancer patch: webView 必须跟随 contentView 尺寸变化（FullSizeContent 切换/
	// 窗口缩放时否则页面整体错位，顶部内容被裁）。
...
void setInvisibleTitleBarHeight(void* window, unsigned int height) {	WebviewWindow* nsWindow = (WebviewWindow*)window;
	// Get delegate
...
// wps-enhancer patch: 定义于 webview_window_darwin.m（跨编译单元 extern 声明）
extern void wpsSetDragZoneHeight(void* nsWindow, unsigned int height);

// wps-enhancer patch: 把 WKWebView 钉回 contentView 当前边界。
// 创建期 contentView 尚为「标题栏以下」区域，FullSizeContentView 稍后才切换为全窗口，
// 期间任何 frame/resize 时序差都会让页面整体偏移（顶部内容被窗口上边缘裁掉）。
// 在全部窗口选项应用完成后调用一次，以 contentView 最终边界为准重新钉正；
// 布局稳定后（0.3s/1.5s）再各重钉一次，覆盖标题栏内缩等晚到的布局调整。
void wpsPinWebViewToContent(void* nsWindow) {
	WebviewWindow* window = (WebviewWindow*)nsWindow;
	NSView* contentView = [window contentView];
	WKWebView* webView = window.webView;
	if (!contentView || !webView) return;
...
void windowSetInvisibleTitleBar(void* nsWindow, unsigned int height) {
	...
	delegate.invisibleTitleBarHeight = height;
	// wps-enhancer patch: 同步拖拽区高度，供 mouseDownCanMoveWindow 判定
	wpsSetDragZoneHeight(nsWindow, height);
}
...
func (w *macosWebviewWindow) run() {
	...
	globalApplication.dispatchOnMainThread(func() {
		...
		// wps-enhancer patch: 选项应用完毕后把 WebView 钉正到最终 contentView 边界
		C.wpsPinWebViewToContent(w.nsWindow)
