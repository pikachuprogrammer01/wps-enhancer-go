...
@property (assign) NSInteger wpsDragZoneHeight; // wps-enhancer patch: 顶部拖拽区高度
...
- (WebviewWindow*) initWithContentRect:(NSRect)contentRect styleMask:(NSUInteger)windowStyle backing:(NSBackingStoreType)bufferingType defer:(BOOL)deferCreation;
{
    ...
    [self setOpaque:NO];
    // wps-enhancer patch: 关闭整窗背景拖动（原 YES 导致任意位置三指/按住都能移动窗口）。
    // 窗口移动仅由 InvisibleTitleBarHeight 顶部拖拽区提供，与寻常 app 行为一致。
    [self setMovableByWindowBackground:NO];
    return self;
}
// wps-enhancer patch: 系统拖移（三指手势等 mouseDownCanMoveWindow 路径）仅顶部拖拽区生效，
// 内容区完全不可拖。顶部区域鼠标拖拽由 WebviewWindowDelegate.handleLeftMouseDown 显式
// performWindowDragWithEvent 处理，不受此限制。
- (BOOL)mouseDownCanMoveWindow {
    if (self.wpsDragZoneHeight > 0) {
        NSPoint location = [self mouseLocationOutsideOfEventStream];
        NSRect frame = [self frame];
        if (location.y > frame.size.height - self.wpsDragZoneHeight) {
            return YES;
        }
    }
    return NO;
}
// wps-enhancer patch: 供 windowSetInvisibleTitleBar（C 侧）同步拖拽区高度
void wpsSetDragZoneHeight(void* nsWindow, unsigned int height) {
    WebviewWindow* window = (WebviewWindow*)nsWindow;
    window.wpsDragZoneHeight = height;
}
