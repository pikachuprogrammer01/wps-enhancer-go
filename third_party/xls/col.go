...
func (c *NumberCol) String(wb *WorkBook) []string {
	if fNo := wb.Xfs[c.Index].formatNo(); fNo != 0 {
		if formatter := wb.Formats[fNo]; formatter != nil {
			// ponytail: patch extrame-xls（停滞库）——General 格式数字被误判为日期（issue #101 变体）
			if strings.ToLower(formatter.str) == "general" || formatter.str == "" {
				return []string{numberString(c.Float)}
			}
...
// numberString 对齐 Python xlrd 数字转字符串规则：整数输出整数，非整数输出浮点文本。
func numberString(v float64) string {
	if v == math.Trunc(v) && v < 1e18 {
